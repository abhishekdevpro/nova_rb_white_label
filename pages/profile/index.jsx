...
const ProfileForm = () => {
    ...
    const fetchData = async () => {
      try { 
        ...
        if (userProfileResponse.data.status === 'success') {
          ...
          // Fetch countries
          const countriesResponse = await axios.get('https://apiwl.novajobs.us/api/user/countries');
  ...
  useEffect(() => {
    const fetchStates = async () => {
      if (formData.country_id) {
        try {
          const token = localStorage.getItem("token");
          const response = await axios.get(`https://apiwl.novajobs.us/api/user/stats/${formData.country_id}`, {
            headers: {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      if (formData.state_id) {
        ...
        try {
          const citiesResponse = await axios.get(`https://apiwl.novajobs.us/api/user/cities/${formData.state_id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch('https://apiwl.novajobs.us/api/jobseeker/user-profile', formDataToSend, {
        headers: {
  ...
  return (
    <div className="p-2 md:p-6">
      ...
      <option disabled>No cities available</option> // Show when no cities are found
          ...
          <button type="submit" className="w-full bg-slate-900 text-white p-2 rounded">Update Profile</button>
        </form>
      </div>
    </div>
  );
