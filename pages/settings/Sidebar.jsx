...
const Sidebar = () => {
  ...
  const getLinkClassName = (path) => {
    return router.pathname === path
      ...
      : "flex items-center p-2 hover:bg-blue-900 hover:text-white border-b-2 rounded font-semibold";
  };
  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          className="md:hidden p-2 fixed top-[7rem] left-4 z-50 bg-blue-900 text-white rounded-full"
          onClick={toggleSidebar}
        >
          <Menu size={24} />
        </button>
        ...
        <ul className="space-y-2 mt-4">
          <li>
            <Link
              ...
              // className="flex items-center p-2 bg-blue-900 border-b-2 border-black font-semibold text-white rounded-xl"
              className={`flex items-center p-2 font-semibold  rounded-xl ${getLinkClassName("/settings/account")}`}
              onClick={toggleSidebar}
            >
