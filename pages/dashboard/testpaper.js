      ...
      const fetchQuestions = async () => {
        try {
          const response = await axios.get(
            `https://apiwl.novajobs.us/api/user/skill-assessment?skill_id=${parseInt(skillId)}&skill_name=${encodeURIComponent(skillName)}`,
            {
              headers: {
                Authorization: token,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(
        `https://apiwl.novajobs.us/api/user/skill-assessment/${skillAssessmentId}`,
        {
      ...
      <div className="font-bold mx-auto text-xl">
        <div className="w-100 d-flex flex-row justify-content-center align-items-center bg-white p-5">
          <div className="w-75 d-flex flex-column align-items-center" style={{ gap: "7px" }}>
            ...
            <h2 className="text-slate-950">Hold On A Second! We re Processing your request...!</h2>
