...
import FullPageLoader from "../../ResumeLoader/Loader";
...
export default function ViewResults({ onBack, formData, scanId }) {
    ...
    try {
      ...
      if (scanId) {
        ...
        response = await axios.post(
          `https://apiwl.novajobs.us/api/user/resume-upload/${scanId}`,
          payload,
          {
            headers: {
        ...
        // ✅ Guest API (POST)
        response = await axios.post("https://apiwl.novajobs.us/api/user/guest-user", payload, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
  ...
  return (
    <div className="text-center">
      {/* ✅ Loader overlay */}
      {loading && <FullPageLoader mode="scanner" isLoading={loading} />}
