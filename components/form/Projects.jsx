...
console.log(resumeData.position,"kkksksk");
  ...
  const handleAutoFixDescription = async (e, projectIndex, content) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://apiwl.novajobs.us/api/user/ai-prosummery",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="flex-col gap-3 w-[90vw]  md:w-full md:mt-10 md:px-10">
      <h2 className="input-title text-black text-3xl mb-2">Projects</h2>
      {resumeData.projects && resumeData.projects.length > 0 ? (
        resumeData.projects?.map((project, projectIndex) => (
          <div
            key={projectIndex}
            className="mb-4 border border-gray-300 bg-white rounded-lg p-4 "
          >
            <div className=" w-full flex justify-between items-center mb-2">
              <h3 className="text-black text-xl font-semibold w-full">
